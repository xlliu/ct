...
class ReadTable(object):
    ...
    def to_kehutongjibiao_mingroup(self, st, et):
        ...
        workbook = xlsxwriter.Workbook(filepath + filetime + 'newcustomerandfollow_g.xlsx')
        worksheet = workbook.add_worksheet()
        worksheet.write(0, 0, unicode('城市', 'utf-8'))
        worksheet.write(0, 1, unicode('大区', 'utf-8'))
        worksheet.write(0, 2, unicode('小组', 'utf-8'))
        worksheet.write(0, 3, unicode('新增客户总数', 'utf-8'))
        worksheet.write(0, 4, unicode('新增400客户数', 'utf-8'))
        worksheet.write(0, 5, unicode('新增主站注册客户数', 'utf-8'))
        worksheet.write(0, 6, unicode('新增电销客户数', 'utf-8'))
        worksheet.write(0, 7, unicode('新增搜房客户数', 'utf-8'))
        worksheet.write(0, 8, unicode('新增安居客客户数', 'utf-8'))
        worksheet.write(0, 9, unicode('新增58客户数', 'utf-8'))
        worksheet.write(0, 10, unicode('新增赶集客户数', 'utf-8'))
        worksheet.write(0, 11, unicode('新增其他网站客户数', 'utf-8'))
        worksheet.write(0, 12, unicode('新增个人客户数', 'utf-8'))
        worksheet.write(0, 13, unicode('新增其他客户数', 'utf-8'))
        worksheet.write(0, 14, unicode('新增老用户客户数', 'utf-8'))
        worksheet.write(0, 15, unicode('新增带看客户数', 'utf-8'))
        row = 1
        for result in cursor.fetchall():
            ...
            row += 1
        workbook.close()
        filename = filetime + 'newcustomerandfollow_g.xlsx'
        return filename


    def check_kehutongjibiao_mingroup(self, d):
        connrd = self.obconn.get_conn()
        cursorrd = connrd.cursor()
        d = cursorrd.execute(
            'delete from new_customerandfollow_mingroup where date=%s', [d]
        )
        print 'delete %s' % d
        connrd.commit()
        self.obconn.close_conn(cursorrd, connrd)
...
class Go(object):
    def set_time(self, st, et):
        print st
        ReadTable().to_kehutongjibiao_mingroup(st, et)
        print "mingroup Ok"
